...
こんにちは！
最近、自分用のツールの個人開発を行なっていたのですが、何となく NestJS を使ってみたくなったんです笑
ということで、NestJS に入門してみました！

本記事では、今回自分が NestJS で開発を行うにあたって調べた内容などを、記事にまとめました。
...
NestJS は、TypeScript で構築された、サーバサイド開発のための Node.js フレームワークです。Angular の影響を受けて開発されているようです。
...
NestJS では、`modular architecture`を採用しています。ですので、機能ごとに１つのモジュールとしてまとめます。機能ごとのモジュールが集まって、１つの NestJS アプリケーションが構成されます。つまり、`Modules`は、機能ごとに関連するルーティングやロジック等をまとめる役割を持つのです。
...
NestJS の使い方が少しわかってきたところで、クライアントサイドから API を叩いてデータの取得や追加ができるようなサービスを想定して、サーバサイドの実装を行なってみたいと思います！
データベースは、Amazon DynamoDB を使っていきます。DynamoDB を選定した理由としては、元々クライアントサイドの方で DynamoDB への項目取得・追加を実装しようとしていて、そのまま使うことにしました。お仕事でも使用経験があったので、多少慣れていたからっていうのもあります。
