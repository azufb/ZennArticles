...
#### オペレーション
GraphQLでは、リクエストのことをオペレーションと呼びます。
Query、Mutation、Subscriptionの3つがあります。

:::message
本記事では、主にQueryとMutationについて説明していきます。
:::

##### Query
データの取得を行うときに使うオペレーションです。

例えば、何かデータを取得(GET)したいときに使います。
...
##### Mutation
データの作成・更新・削除を行うときに使うオペレーションです。

例えば、何かをPOSTしたりするような場合に使います。

```graphql
mutation postSomething($id: ID) {
    something(id: $id) {
        name
        comment
    }
...
#### スキーマを作る
スキーマを作ってみます。
今回は、オブジェクト型とQuery型・Mutation型の3パターンでやってみます。
...
```
実行すると、``hello``と``getSampleUsers``の2つのQueryの返り値の型定義が行われています。
``hello``は文字列を返すのでString型で定義されており、``getSampleUsers``は``SampleUser``型の配列(``sampleUsers``)を返すので、``[sampleUser]``というように配列で返すことがわかるように定義されています。
...
``sampleUsers``を増やす、``postSampleUser``というMutationです。引数を使っています。
...
次に、こちらもQueryの``getSampleUsers``です。
``getSampleUsers``を実行すると、配列``sampleUsers``が全て返ってきます。
...
##### 【Mutation】postSampleUser
最後は、Mutationの``postSampleUser``です。
``postSampleUser``を実行すると、``name``と``comment``をプロパティとして持つオブジェクトを引数として渡し、配列``sampleUsers``に追加を行なっています。
追加したオブジェクトを返します。
...
###### 【Query】getSampleUsers
続いて、Queryの``getSampleUsers``を実行します。
GraphiQLの画面左側に以下のように記述します。
...
``hello``とは異なり、``getSampleUsers``の場合は取得するデータを指定することができます。上記だと、オブジェクト型``SampleUser``のオブジェクトが持つ全てのプロパティ(``id``・``name``・``comment``)を返しています。
実行すると、次のようになります。
![](https://storage.googleapis.com/zenn-user-upload/cb846777be21-20230917.png)
...
実行すると、次のようになります。
![](https://storage.googleapis.com/zenn-user-upload/ce4f0b7433ab-20230917.png)

``name``プロパティのみを持つオブジェクトを要素とする配列が返ってきました。

###### 【Mutation】postSampleUser
Mutationの``postSampleUser``を実行します。
GraphiQLの画面左側に次のように記述します。

```graphql
mutation postSampleUser ($name: String, $comment: String) {
  postSampleUser (name: $name, comment: $comment) {
    id
    name
    comment
  }
...
これで実行すると、次のようになります。
![](https://storage.googleapis.com/zenn-user-upload/e5203781b680-20230917.png)

追加された、``name``が``user_4``で、``comment``が``Hi!``のオブジェクトが返ってきています。``id``もきちんと``配列の要素の数+1``になっているようです。
ここでももちろん、返すデータを減らすことが可能です。

念の為、追加されたのかを確認しておきます。
Queryの``getSampleUsers``を先ほどの説明と同じようにやってみます。すると、次のようになります。
![](https://storage.googleapis.com/zenn-user-upload/b3af0d0d76bb-20230917.png)

先ほど追加した内容で、要素が1つ増えていることがわかります。
