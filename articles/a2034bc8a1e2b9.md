...
- スタイリングには、Emotionというライブラリを使用していますが、今回はそのあたりについては触れておりません。
...
# Recoilとの出会い
業務では状態管理にReduxを使っているのですが、Recoilに置き換えることになって、勉強しておこうと思ったことがきっかけでした。

# Recoilとは
Recoilとは、Mata(Facebook)が開発を行なっている、Reactの状態管理ライブラリの1つです。
...
`atom`は、加工されていない、純粋な・正式なstateの値を管理しています。(なぜ、このような言い方をしているのかは次の項目で何となくわかるかも？)
